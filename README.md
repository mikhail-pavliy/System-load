...
В итоге у нас есть сконфгиурированный vagrant файл и два скрипта прикрепленных в этом репо
1. scripts/grub.sh добавляет репозиторий с пропатченным grub и обновляет его
2. scripts/fsmove.sh создает lvm на доп. диске переносит туда файлы системы с текущего lvm + boot раздел, далее в chroot на новом разделе меняет fstab, устанавливает grub на новый 
диск, меняет grub и пересобирает initramfs.
